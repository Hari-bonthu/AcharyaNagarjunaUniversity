...
const quickLinks = [
  ...
  { label: "Careers", href: "/pages/employee-services?page=careers" },
...
const portalLinks = [
  ...
  { label: "WebMail", href: "/pages/employee-services?page=webmail" },
];
      ...
      >
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center justify-between gap-x-6 gap-y-2 px-6 py-2.5 lg:px-8">
          ...
          <div className="hidden items-center gap-5 lg:flex">
            {quickLinks.map((link) => (
              <a key={link.label} href={link.href} className="font-medium hover:underline" style={{ color: "var(--utility-bar-accent)" }}>
                {link.label}
              </a>
            ))}
      ...
      <div className="w-full bg-background">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-6 px-6 py-6 md:grid-cols-[auto_1fr_auto]">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold tracking-wide text-[#d71920] md:text-4xl">ఆచార్య నాగార్జున విశ్వవిద్యాలయం</p>
            <h1 className="mt-1 text-2xl font-bold tracking-wide text-[oklch(0.22_0.06_265)] md:text-4xl">
              ACHARYA NAGARJUNA UNIVERSITY
            </h1>
          </div>
          <div className="mx-auto hidden h-8 w-8 md:block" />
        </div>
      </div>
